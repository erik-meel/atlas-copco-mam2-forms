...
export class AppComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    this.formService.getInput().subscribe((formInput: MelVariable) => {
        this.context.run(() => {
          ...
          this.language = 'SKD_SK'; //this.INPUT.get('LANG') ? this.INPUT.get('LANG') : currentLanguage;
  ...
  onCustomerSignatureEnd(): void {
    this.customerSignatureDate = (new Date()).toLocaleString('sk_SK', this.formService.dateTimeFormat);
  ...
  onTechSignatureEnd(): void {
    this.techSignatureDate = (new Date()).toLocaleString('sk_SK', this.formService.dateTimeFormat);
    this.formService.saveHtmlToMel();
  }
