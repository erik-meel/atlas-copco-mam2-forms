...
export class AppComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    this.formService.getInput().subscribe((formInput: MelVariable) => {
        this.context.run(() => {
          ...
          this.language = 'SIO_SI'; //this.INPUT.get('LANG') ? this.INPUT.get('LANG') : currentLanguage;
  ...
  onCustomerSignatureEnd(): void {
    this.customerSignatureDate = (new Date()).toLocaleString('si', this.formService.dateTimeFormat);
  ...
  onTechSignatureEnd(): void {
    this.techSignatureDate = (new Date()).toLocaleString('si', this.formService.dateTimeFormat);
    this.formService.saveHtmlToMel();
  }
