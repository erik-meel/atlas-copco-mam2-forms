...
export class AppComponent implements OnInit, AfterViewInit {
  ...
  private dateMask = '';
  private language = 'NLA_NL'; //currentLanguage;
  ...
  private STATIC: MelVariable = new MelVariable ({
    ...
    'MEASUREMENT_POINTS_ENTRIES_EXTRA': {
        ...
        '09': {
            'COUNTER': 'Behaald vacuüm < 2000micron (270 Pa)',
            'READ_AFTER': '',
            'READ_UNIT': '',
            'READ_BEFORE': '',
            'READ_LAST_VISIT': '',
        ...
        '10': {
            'COUNTER': 'Standtijd vacuüm (> 30 min.)',
            'READ_AFTER': '',
            'READ_UNIT': 'min.',
            'READ_BEFORE': '',
            'READ_LAST_VISIT': '',
        ...
        '11': {
            'COUNTER': 'Controle lekdichtheid na rep.<1mnd ',
            'READ_AFTER': '',
            'READ_UNIT': '',
            'READ_BEFORE': '',
            'READ_LAST_VISIT': '',
  ...
  ngOnInit() {
    this.formService.getInput().subscribe((formInput: MelVariable) => {
        this.context.run(() => {
          ...
          this.language = 'NLA_NL'; //this.INPUT.get('LANG') ? this.INPUT.get('LANG') : currentLanguage;
  ...
  onCustomerSignatureEnd(): void {
    this.customerSignatureDate = (new Date()).toLocaleString("NL-nl", this.formService.dateTimeFormat);
  ...
  onTechSignatureEnd(): void {
    this.techSignatureDate = (new Date()).toLocaleString("NL-nl", this.formService.dateTimeFormat);
    this.formService.saveHtmlToMel();
  }
