  ...
  ngOnInit() {
    this.formService.getInput().subscribe((formInput: MelVariable) => {
        this.context.run(() => {
          ...
          let arr = this.INPUT.getVarArray('SAFETY_CHECK_LIST_ENTRIES');
          for(let safety of arr)
           {
             if(safety.tag('DONE').length > 0) {
               this.LMRA_enabled = true;
             }
           }
        });
      },
