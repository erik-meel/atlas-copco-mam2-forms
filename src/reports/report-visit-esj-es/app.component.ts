...
export class AppComponent implements OnInit, AfterViewInit {
  ...
  //Added new condition by Akhila to check if . is present then only take substring else take the value as it is .
    var round=name;
    if(round.includes('.')){
     round = name.substring(0, name.indexOf('.'));
  }
    return round;
  }
