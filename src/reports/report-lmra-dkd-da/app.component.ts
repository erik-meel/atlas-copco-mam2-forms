...
export class AppComponent implements OnInit, AfterViewInit {
  ...
  private prevLanguage = this.language;
  ...
  ngOnInit() {
    this.formService.getInput().subscribe((formInput: MelVariable) => {
        this.context.run(() => {
          ...
          let PlanGroup = this.INPUT.get('PLANGROUP');
          
          if(PlanGroup=="Z01") {
            this.language = 'DKD_DA'; //this.INPUT.get('LANG') ? this.INPUT.get('LANG') : currentLanguage;
            this.dateLocale = 'da-DK';
          } else if(PlanGroup=="Z02") {
            this.language = 'DKD_SK'; //this.INPUT.get('LANG') ? this.INPUT.get('LANG') : currentLanguage;
            this.dateLocale = 'sk-SK';
          } else {
            this.language = 'DKD_EN'; //this.INPUT.get('LANG') ? this.INPUT.get('LANG') : currentLanguage;
  ...
  onCustomerSignatureEnd(): void {
    this.customerSignatureDate = (this.dateLocale);
  ...
  onTechSignatureEnd(): void {
    this.techSignatureDate = (this.dateLocale);
    this.formService.saveHtmlToMel();
  }
